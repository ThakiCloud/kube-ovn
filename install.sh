...
ENABLE_BAREMETAL=${ENABLE_BAREMETAL:-false}
if [[ $ENABLE_BAREMETAL = "true" ]];then
  EXTERNAL_KUBE_API_SERVER_HOST=${EXTERNAL_KUBE_API_SERVER_HOST:-}
  EXTERNAL_OVN_DB_IP=${EXTERNAL_OVN_DB_IP:-}
  echo "kube-api-server external ip : $EXTERNAL_KUBE_API_SERVER_HOST"
  echo "ovn-db external ip : $EXTERNAL_OVN_DB_IP"
fi
...
apiVersion: apps/v1
metadata:
  name: ovs-ovn-baremetal
  namespace: kube-system
  annotations:
    kubernetes.io/description: |
      This daemon set launches the openvswitch daemon (external OVN DB group).
spec:
  selector:
    matchLabels:
      app: ovs-baremetal
...
apiVersion: apps/v1
metadata:
  name: kube-ovn-pinger-baremetal
  namespace: kube-system
  annotations:
    kubernetes.io/description: |
      This daemon set launches the pinger daemon (external API group).
spec:
  selector:
    matchLabels:
      app: kube-ovn-pinger-baremetal
  updateStrategy:
    type: RollingUpdate
  template:
    metadata:
